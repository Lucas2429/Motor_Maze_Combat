...
@export var map_select: PackedScene
...
func _ready() -> void:
	if Game.first_game:
		...
		Debug.log("Primer juego")
		mapa_escogido(4)
	else:
		mapa_escogido(Game.map_chosen)

func mapa_escogido(id: int) -> void:
	if multiplayer.is_server():
		if id == 4:
			Debug.log("Mapa Random")
			randomize()
			indice_mapa_elegido = randi() % lista_de_mapas.size()
			cargar_mapa_por_indice(indice_mapa_elegido)
		else:
			indice_mapa_elegido = id - 1
			Debug.log("Mapa elegido: " + str(indice_mapa_elegido + 1))
			cargar_mapa_por_indice(indice_mapa_elegido)
	else:
